...
#############################
def get_band_power_segment(raw):
    ...
    try:
        ...
        # print(f"band power results:\n{band_dict}")
        flag = True
    except:
        print("An exception occurred") 
        flag=False
    
    return band_dict, flag 
...
def main(args):
    global spectrum, data_spectrum, fig, ax, ani, draw_image, frame_slider, data_eeg, raw_closed_eyes, ax_topoplot, axfreq, fig_topoplot, cbar_ax, sampling_rate, raw_data, arr_psd, y_limits
    ...
    if abt == 0:
        ## choose this option when resting and biking recordings were saved in the same file
        ...
        ################################
        ## Stage 1: band pass filter (in place)
        #################################
        low_cut =    0.5
        hi_cut  =   45.0
        ...
        ## choose this option when resting and biking recordings were saved in two different files
    ...
    #######################
    # visualization before re-referencing
    # section = 'baseline'
    # id_section = 0
    # moment = 'before re-refrencing'
    # eeg_data_before = eeg_data_dict[section][id_section].copy()
    # mne.viz.plot_raw(eeg_data_before, start=0, duration=240, scalings=scale_dict, title=f"{section, id_section, moment}", block=False)
    ...
    #######################
    # moment = 'after re-refrencing'
    # mne.viz.plot_raw(eeg_data_dict[section][id_section], start=0, duration=240, scalings=scale_dict, title=f"{section, id_section, moment}", block=True)
    ...
    #########################
    ## EEG signals visualization before ICA
    # 
    # fig_psd, ax_psd = plt.subplots(2,1)
    # #             
    # section = 'a_opened_eyes'
    # id_section = 0
    # moment = 'before ICA'

    # mne.viz.plot_raw(eeg_data_dict[section][id_section], start=0, duration=240, scalings=scale_dict, title=f"{section, id_section, moment}", block=False)

    
    # mne.viz.plot_raw_psd(eeg_data_dict[section][id_section], ax= ax_psd[0], exclude=[], fmax=100, reject_by_annotation=True, xscale='log', )
    # ax_psd[0].set_title(moment)
    ## visualization
    #########################

    ## ICA for blink removal using precalculate ICA (ica_blinks.py)
    ## remove blinks only from baseline (when only ICA was applied to baseline)
    # eeg_data_dict = ica_baseline_func(path, eeg_data_dict, subject, session, scale_dict)

    ## remove blinks from all segments (when ICA was applied to each and every segment... including baseline, opened-eyes, closed-eyes)
    ...
    # mne.viz.plot_raw_psd(eeg_data_dict[section][id_section], ax= ax_psd[1], exclude=[], fmax=100, reject_by_annotation=True, xscale='log', )
    # ax_psd[1].set_title(moment)
    ...
    ###########################
    ## baseline band power of reference for normalization
    ...
    # from segments of 5 s
    # freq. bands theta (4-8 Hz), alpha (8-12 Hz), beta (12-30 Hz)
    
    ## create folder if it does not exit already
    Path(f"{path}session_{session}/figures/topomaps/sequence/").mkdir(parents=True, exist_ok=True)
    
    filename_fig = f"{path}session_{session}/figures/topomaps/sequence/"
    # y_limits = (None, None)

    section = 'b_closed_eyes'
    id_section = 0

    eeg_data = eeg_data_dict[section][id_section].copy()

    print(f"{eeg_data.info}")
    print(f"duration: {eeg_data.duration}")
    print(f"times: {eeg_data.times}")
    print(f"n_times: {eeg_data.n_times}")

    # fig_an, ax_an = plt.subplots()
    # ims = []

    t0    =  0.0   ## in seconds
    delta =  5.0   ## in seconds
    step  =  1.0   ## in seconds

    sample_list = np.arange(0, eeg_data.duration, step)

    for i, t0 in enumerate(sample_list):

        t1 = t0 + delta
        print(f"t0, t1: {t0, t1}")

        if t1 < (eeg_data.duration-0.1):
            eeg_piece = eeg_data.copy().crop(tmin=t0, tmax=t1)
            
            moment = 'after crop'

            bp_dict, flag = get_band_power_segment(eeg_piece)
            print(f"flag: {flag}")

            if flag==True:
                bp_norm_dict = get_normalized_band_power(bp_dict, bp_baseline_dict)

                fig_bands = plot_topomap_powerbands(bp_norm_dict, eeg_data_dict, f"{section, id_section, moment}", filename_fig)

                fig_bands.savefig(f"{filename_fig}fig_{str(i).zfill(3)}.png", transparent=False)
    ...
    # mne.viz.plot_raw(eeg_piece, start=0, duration=240, scalings=scale_dict, title=f"{section, id_section, moment}", block=True)
    ...
    # eeg_data = eeg_data_dict[section][id_section].copy()
    # bp_dict = get_band_power_segment(eeg_data)

    # plot_topomap_powerbands(bp_dict, eeg_data_dict, f"{section, id_section}", filename_fig)
