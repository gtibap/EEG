...
def plot_eeg_bands(sessions_values_dict, title):
    ...
    for label_values in sessions_values_dict:
        ...
        for alpha_label in alpha_dict:
            if 'ce_left' in alpha_label:
                axs[0].bar(id_s, alpha_dict[alpha_label], label=bar_labels[id_s], color=bar_colors[id_s])
            elif 'ce_right' in alpha_label:
                axs[2].bar(id_s, alpha_dict[alpha_label], color=bar_colors[id_s])
            elif 'oe_left' in alpha_label:
                axs[4].bar(id_s, alpha_dict[alpha_label], color=bar_colors[id_s])
            elif 'oe_right' in alpha_label:
                axs[6].bar(id_s, alpha_dict[alpha_label], color=bar_colors[id_s])
            else:
                pass

        for beta_label in beta_dict:
            if 'ce_left' in beta_label:
                axs[1].bar(id_s, beta_dict[beta_label], color=bar_colors[id_s])
            elif 'ce_right' in beta_label:
                axs[3].bar(id_s, beta_dict[beta_label], color=bar_colors[id_s])
            elif 'oe_left' in beta_label:
                axs[5].bar(id_s, beta_dict[beta_label], color=bar_colors[id_s])
...
def create_axes():
    ...
    for id, ax in enumerate(axs):
        ...
        if id in [0,2]:
            ax.annotate(f'alpha band', xy=(0, 0), xycoords='data', xytext=(0.01, .99), textcoords='axes fraction', va='top', ha='left',)
        elif id in [1,3]:
            ax.annotate(f'beta band', xy=(0, 0), xycoords='data', xytext=(0.01, .99), textcoords='axes fraction', va='top', ha='left',)
        elif id in [4,6]:
            ax.annotate(f'alpha band', xy=(0, 0), xycoords='data', xytext=(0.01, .99), textcoords='axes fraction', va='top', ha='left',)
        elif id in [5,7]:
            ax.annotate(f'beta band', xy=(0, 0), xycoords='data', xytext=(0.01, .99), textcoords='axes fraction', va='top', ha='left',)
        ...
        if id in [0,1]:
            ...
            ann = ax.annotate(f'eyes closed\nleft central side', xy=(0, 1.05 ), xycoords=trans)
        elif id in [2,3]:
            trans = ax.get_xaxis_transform() # x in data untis, y in axes fraction
            ann = ax.annotate(f'eyes closed\nright central side', xy=(0, 1.05 ), xycoords=trans)
        elif id in [4,5]:
            trans = ax.get_xaxis_transform() # x in data untis, y in axes fraction
            ann = ax.annotate(f'eyes open\nleft central side', xy=(0, 1.05 ), xycoords=trans)
        elif id in [6,7]:
            trans = ax.get_xaxis_transform() # x in data untis, y in axes fraction
            ann = ax.annotate(f'eyes open\nright central side', xy=(0, 1.05 ), xycoords=trans)
