...
def main(args):
    ...
    print(f"epochs.selection: {epochs.selection.tolist(), type(epochs.selection.tolist())}")
    # print(f"drop log:\n{epochs.drop_log}")

    ## use epochs to create obj from class Epochs_Processing{}
    bad_epochs_list = np.array([20,22,24])
    ids_bad_list = []
    for id_bad in bad_epochs_list:
        drop_idx = np.where(epochs.selection == id_bad)[0]
        ids_bad_list = np.concatenate((ids_bad_list, drop_idx), axis=None)

    print(f"ids_bad_list: {ids_bad_list}")

    epochs.drop(ids_bad_list.astype(int))

    print(f"epochs:\n{epochs}")

    ## save selected epochs
    print(f"epochs.selection: {epochs.selection.tolist(), type(epochs.selection.tolist())}")

    sel_bad_epochs={
        "ids": epochs.selection.tolist(),
    }
    with open(f"{path_prep}{label}_selected_epochs.json", "w") as f:
        json.dump(sel_bad_epochs, f)
    ...
    # print(f"epochs.drop_log: {epochs.drop_log}")
    epochs.plot(n_epochs=12, events=True, block=True, n_channels=36, scalings=scale_dict, title=f"EEG epochs after time series",)
    ...
    # print(f'updated annotations:\n{updated_annot}')
    # ## adding annotations to raw data
    # raw_data.set_annotations(updated_annot)

    # fig_raw = mne.viz.plot_raw(raw_data, picks=['eeg','ecg'], start=0, duration=240, n_channels=36, scalings=scale_dict, highpass=1.0, lowpass=45.0, title=f"EEG {labels_list[0]} -- new annotations", block=True)
    ...
    ## first, include periodic events to the raw data 
    new_events = mne.make_fixed_length_events(raw_data, start=0.5, stop=None, duration=dt)
    raw_data.add_events(new_events, replace=True)

    ## second, use events to create epochs
    epochs = mne.Epochs(raw_data, new_events, tmin=0.0, tmax=dt, baseline=None, preload=True, reject=None, reject_by_annotation=True)
    print(f"epochs:\n{epochs}")
    # print(f"drop log:\n{epochs.drop_log}")
    ...
    epochs.plot(n_epochs=12, events=True, block=True, n_channels=36, scalings=scale_dict, title=f"EEG epochs time series",)
